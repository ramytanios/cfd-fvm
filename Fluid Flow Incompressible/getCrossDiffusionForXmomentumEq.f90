...
! the source term.
! *************************************************
subroutine getCrossDiffusionForXmomentumEq()
...
do i = 1,m-1
	...
	if (i.eq.1) then
		x0fgradE = x0fGradu(i+1,j)*EastGeomFactor(i,j) + x0fGradu(i,j)*(1-EastGeomFactor(i,j))
		y0fgradE = y0fGradu(i+1,j)*EastGeomFactor(i,j) + y0fGradu(i,j)*(1-EastGeomFactor(i,j))
		if (vonNeumannWest(j) .OR. robinWest(j)) then
			x0fgradW = 0.0D0
			y0fgradW = 0.0D0
		else
			x0fgradW = x0fGradu(i,j)
			y0fgradW = y0fGradu(i,j)
		end if
	else if (i.eq.m-1) then
		x0fgradW = x0fGradu(i-1,j)*WestGeomFactor(i,j) + x0fGradu(i,j)*(1-WestGeomFactor(i,j))
		y0fgradW = y0fGradu(i-1,j)*WestGeomFactor(i,j) + y0fGradu(i,j)*(1-WestGeomFactor(i,j))
	...
	if (j.eq.1) then
		x0fgradN = x0fGradu(i,j+1)*NorthGeomFactor(i,j) + x0fGradu(i,j)*(1-NorthGeomFactor(i,j))
		y0fgradN = y0fGradu(i,j+1)*NorthGeomFactor(i,j) + y0fGradu(i,j)*(1-NorthGeomFactor(i,j))
		if (vonNeumannSouth(i) .OR. robinSouth(i)) then
			x0fgradS = 0.0D0
			y0fgradS = 0.0D0
		else
			x0fgradS = x0fGradu(i,j)
			y0fgradS = y0fGradu(i,j)
		end if
	else if (j.eq.n-1) then
		x0fgradS = x0fGradu(i,j-1)*NorthGeomFactor(i,j) + x0fGradu(i,j)*(1-NorthGeomFactor(i,j))
		y0fgradS = y0fGradu(i,j-1)*NorthGeomFactor(i,j) + y0fGradu(i,j)*(1-NorthGeomFactor(i,j))
		if (vonNeumannNorth(i) .OR. robinNorth(i)) then
			x0fgradN = 0.0D0
			y0fgradN = 0.0D0
		else
			x0fgradN = x0fGradu(i,j)
			y0fgradN = y0fGradu(i,j)
		end if
