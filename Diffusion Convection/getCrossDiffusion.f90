...
do i = 1,m-1
	...
	if (i.eq.1) then
		...
		if (vonNeumann(2) .OR. robin(2)) then
			x0fgradW = 0.0D0
			y0fgradW = 0.0D0
		else
		...
		if (vonNeumann(4) .OR. robin(4)) then
			x0fgradE = 0.0D0
			y0fgradE = 0.0D0
		else
	...
	if (j.eq.1) then
		...
		if (vonNeumann(3) .OR. robin(3)) then
			x0fgradS = 0.0D0
			y0fgradS = 0.0D0
		else
		...
		if (vonNeumann(1) .OR. robin(1)) then
			x0fgradN = 0.0D0
			y0fgradN = 0.0D0
		else
	...
	bCmodified(i,j) = bCmodified(i,j) + crossDiffTerm
