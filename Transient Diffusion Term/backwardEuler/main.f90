...
open(unit=1,file="dataBackward.txt")

do i = 1,m-1
	do j = 1,n-1
		write(1,*) x0fCellCentroid(i,j),achar(9),y0fCellCentroid(i,j),achar(9),&
							phi(i,j),phiAnalytical(i,j),100*abs(phi(i,j)-phiAnalytical(i,j))/phiAnalytical(i,j)
	end do
end do
